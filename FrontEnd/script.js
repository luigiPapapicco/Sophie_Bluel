...
// récuperation de l'input "objets"
const objetsBtnFilter = document.querySelector("input[name='Objets']");
objetsBtnFilter.addEventListener("click", () => {
        const objetsFiltres = works.filter(function (objet) {
            return objet.category.name === "Objets";
        })
        console.log(objetsFiltres)

        // clean de la section gallery pour un appel a la fonction avec la liste trier
        document.querySelector(".gallery").innerHTML = "";
        genererWorks(objetsFiltres)
})


// récuperation de l'input "Appartements"
const appartementsBtnFilter = document.querySelector("input[name='Appartements']");
appartementsBtnFilter.addEventListener("click", () => {
        const appartementFiltres = works.filter(function (appartements) {
            return appartements.category.name === "Appartements";
        })
        console.log(appartementFiltres)

        // clean de la section gallery pour un appel a la fonction avec la liste trier
        document.querySelector(".gallery").innerHTML = "";
        genererWorks(appartementFiltres)
})

// récuperation de l'input "Hotels & restaurants"
const hotelEtRestaurantBtnFilter = document.querySelector("input[name='Hotels & restaurants']");

hotelEtRestaurantBtnFilter.addEventListener("click", () => {
        const appRestFiltres = works.filter(function (appRest) {
            return appRest.category.name === "Hotels & restaurants";
        })
        console.log(appRestFiltres)

        // clean de la section gallery pour un appel a la fonction avec la liste trier
        document.querySelector(".gallery").innerHTML = "";
        genererWorks(appRestFiltres)
})
